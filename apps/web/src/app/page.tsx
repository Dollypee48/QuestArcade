    ...
    <div className="mt-12">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <h3 className="text-2xl font-semibold text-foreground">Featured Quests</h3>
          <p className="mt-1 text-sm text-foreground/70">Top quests with highest rewards</p>
        </div>
        <Button asChild variant="ghost" className="gap-2 text-foreground/80 hover:text-foreground">
          <Link href="/quests">
            View All
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {featuredQuests.map((quest, index) => {
          ...
          return (
            ...
            >
              <Card className="glass-card group h-full border-white/10 bg-white/5 p-6 shadow-glow-sm transition-all hover:border-white/20 hover:shadow-glow">
                <CardContent className="p-0">
                    <div className="mb  -4 flex items-start justify-between">
                      ...
                      <Badge
                        variant="accent"
                        className="border border-white/20 bg-white/5 text-xs text-foreground/80"
                      >
                        {quest.difficulty}
                      </Badge>
                    </div>
                  <h4 className="mb-2 text-lg font-semibold text-foreground">{quest.title}</h4>
                  <p className="mb-3 line-clamp-2 text-sm text-foreground/70">{quest.description}</p>
...
<main className="flex-1">
        ...
        <div className="container relative z-10 mx-auto max-w-6xl px-4 text-center">
          ...
          >
            ...
            <Button
              ...
              size="lg"
              variant="ghost"
              className="gap-2 rounded-full border border-white/20 bg-white/5 px-8 py-6 text-base text-foreground/80 hover:bg-white/10"
            >
              <Link href="/about">
                How It Works
            ...
            {isConnected && address ? (
              ...
              <Button
                asChild
                variant="outline"
                className="gap-2 rounded-full border-white/20 bg-white/10 px-8 py-6 text-base text-foreground/85 hover:bg-white/20"
              >
                <Link href="/login">
                  Connect Wallet
                  <Wallet className="h-4 w-4" />
            ...
            <div className="space-y-6 lg:w-1/2">
              ...
              <div className="grid gap-4">
                {featureCards.map((feature) => (
                  <div
                    key={feature.title}
                    className="glass-card flex items-start gap-4 rounded-2xl border border-white/10 bg-white/5 p-5"
                  >
      ...
      <section className="section-padding bg-background/40">
        <div className="container mx-auto max-w-6xl px-4">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-center text-sm text-foreground/70">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {howItWorks.map((item) => (
              <motion.div
                key={item.label}
                className="glass-card flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-8"
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
      ...
      <section className="section-padding">
        <div className="container mx-auto max-w-5xl rounded-[32px] border border-white/10 bg-gradient-secondary px-6 py-16 text-center shadow-glow lg:px-12">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-sm text-foreground/75">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
            ...
            <Button
              ...
              size="lg"
              variant="ghost"
              className="rounded-full border border-white/20 bg-white/5 px-8 py-6 text-foreground/85 hover:bg-white/10"
            >
